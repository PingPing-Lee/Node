...
function graphqlHTTP(options) {
  ...
  return async function middleware(ctx) {
    ...
    try {
      ...
      executeFn = optionsData.customExecuteFn || _graphql.execute;
      ...
      result = await new Promise(function (resolve) {
        ...
        try {
          resolve(executeFn(schema, documentAST, rootValue, context, variables, operationName, fieldResolver));
          response.status = 200;
        } catch (contextError) {
    ...
    if (result && result.errors) {
      result.errors = result.errors.map(function (err) {
        return formatErrorFn ? formatErrorFn(err, context) : (0, _graphql.formatError)(err);
      });
    }

    // If allowed to show GraphiQL, present it instead of JSON.
    if (showGraphiQL) {
      var payload = (0, _renderGraphiQL.renderGraphiQL)(Object.assign({
        query: query,
        variables: variables,
        operationName: operationName,
        result: result
      ...
      var _payload = pretty ? JSON.stringify(result, null, 2) : result;
